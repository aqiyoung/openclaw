...
<AccordionGroup>
  <Accordion title="Channel subpaths">
    ...
    | `plugin-sdk/bundled-channel-config-schema` | Private-local after July 2026; bundled OpenClaw channel config schemas for maintained bundled plugins only. The deprecated `DiscordConfigSchema` and `MSTeamsConfigSchema` exports temporarily support official plugins published before `2026.7.2-beta.4`; remove them once the supported official-plugin update floor excludes those versions. |
