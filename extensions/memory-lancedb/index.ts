 ...
 * leaked outside its ```json fence). Allows `{` on its own line so pretty-printed
 * JSON (`formatContextJsonBlock`'s `JSON.stringify(..., null, 2)` output) is also
 * caught. Key list mirrors envelope identifiers used by
